...
class HIDController:
    ...
    # Track last mouse position for absolute positioning
    # For absolute mouse HID, coordinates must be included in EVERY report
    _last_x = 16383  # Start at center
    _last_y = 16383
    ...
    def mouse_move(x, y):
        ...
        # Track position for use in click reports
        HIDController._last_x = x
        HIDController._last_y = y
    ...
    def mouse_click(button='left', double=False, x=None, y=None):
        ...
        IMPORTANT: For absolute mouse HID, coordinates MUST be included in
        every report, including button press/release. Otherwise the cursor
        jumps to 0,0.
        """
        button_map = {'left': 0x01, 'right': 0x02, 'middle': 0x04}
        btn = button_map.get(button, 0x01)

        # Use provided coordinates or last known position
        if x is not None and y is not None:
            click_x = max(0, min(MAX_COORD, int(x)))
            click_y = max(0, min(MAX_COORD, int(y)))
            HIDController._last_x = click_x
            HIDController._last_y = click_y
        else:
            click_x = HIDController._last_x
            click_y = HIDController._last_y

        with open(MOUSE_DEVICE, 'wb') as f:
            # Click down - include coordinates to maintain position
            f.write(struct.pack('<Bhhbb', btn, click_x, click_y, 0, 0))
            f.flush()
            time.sleep(0.02)
            # Click up - include coordinates to maintain position
            f.write(struct.pack('<Bhhbb', 0, click_x, click_y, 0, 0))
            f.flush()
    ...
    def mouse_scroll(amount):
        ...
        # Include position in scroll report to maintain cursor position
        report = struct.pack('<Bhhbb', 0, HIDController._last_x, HIDController._last_y, amount, 0)
        with open(MOUSE_DEVICE, 'wb') as f:
            f.write(report)
...
class HIDRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == '/health':
            self.send_json({'status': 'ok', 'service': 'kvm-hid-server', 'version': '2.0'})
        ...
        try:
            if path == '/mouse/move':
                ...
                self.send_json({
                    ...
                    'action': 'mouse_click',
                    'button': button,
                    'x': x if x is not None else HIDController._last_x,
                    'y': y if y is not None else HIDController._last_y,
                })
...
def main():
    ...
    if args.tls:
        ...
        print(f'KVM HID Server v2.0 listening on https://{args.host}:{args.port}')
    else:
        print(f'KVM HID Server v2.0 listening on http://{args.host}:{args.port}')
